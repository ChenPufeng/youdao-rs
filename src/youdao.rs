...
fn parser2(html:&String) -> Vec<String> {
    ...
    if trans["errorCode"].as_i64().unwrap() == 0 {
        ...
        // chinese
		let base = trans["basic"].as_object().unwrap();
    	...
    	//let base = trans["web"].as_object().unwrap();
...
pub fn query2(word:String) -> Vec<String> {
    ...
    let url_str: String = format!("http://fanyi.youdao.com/openapi.do?keyfrom=f2ec-org&key=1787962561&type=data&doctype=json&version=1.1&q={}", w);
